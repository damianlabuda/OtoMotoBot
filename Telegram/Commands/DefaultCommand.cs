    ...
    public class DefaultCommand : IBaseCommand, IDefaultCommand
    {
        ...
        public async Task ExecuteAsync(Update update)
        {
            ...
            if (update.Type == UpdateType.Message)
            {
                ...
                await _redis.Connection.UnlinkAsync($"{typeof(TelegramCurrentActionRedis)}:{update.Message.Chat.Id}");
                return;
            }
            ...
            if (update.Type == UpdateType.CallbackQuery)
            {
                ...
                await _redis.Connection.UnlinkAsync($"{typeof(TelegramCurrentActionRedis)}:{update.CallbackQuery.Message.Chat.Id}");
                return;
            }
        }
    }
